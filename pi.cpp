...
void pi_sum(char *key, int keybytes,
  ...
  auto processpage = [&] (char *multivalue, int nvalue, int *valuebytes) {
    char* base = multivalue;
    for (int valind=0; valind<nvalue; ++valind) {
  ...
  if (multivalue != nullptr)
        processpage(multivalue, nvalue, valuebytes);
  else {
    MAPREDUCE_NS::MapReduce* mr = (MAPREDUCE_NS::MapReduce*)valuebytes;
    int nbblocks;
    mr->multivalue_blocks(nbblocks); //writes to nbblocks
    for (int iblock=0; iblock<nbblocks; ++iblock) {
      nvalue = mr->multivalue_block(iblock,&multivalue,&valuebytes); //reading a page
      processpage(multivalue, nvalue, valuebytes);
    }
  }

  kvout->add (key, keybytes, (char*)&count, sizeof(count));  //generating output
}
